...
- ✅ 课表 JSON（`kbList`）→ ICS，采用**紧凑 RRULE 周期事件**（一门课/一个时段一条 VEVENT，整学期约 21 条、文件 ~10KB），支持：
  - 周次 `1-16周`、`13-16周`、`1-8周,10-16周`（自动拆成多条周期）
  - 单双周 `2-4周(双)` / `3-5周(单)` —— 用 `RRULE:FREQ=WEEKLY;INTERVAL=2` 表达，日期由学期第 1 周周一锚点精确推出，互不重叠
  - 一次连上多节（`5-8`、`1-4`），起止时间取首末节次（节次→时间表来自 `cxRjc` 实时下发）
- ✅ 线上课忽略：只取有排课的 `kbList`，无教室/UOOC 线上课不进日历
- ✅ **稳定 UID 防重复**：UID = `学期+课程代码+星期+节次+周次序列`（不含教室/教师/时间表）；换地点、换老师、改打铃时间不产生新 UID；真正换时段/周次的，旧时段整条以 `STATUS:CANCELLED` 发出（保留原 RRULE），订阅端不会残留重复
...
│  ├─ icsgen.py              # RRULE 周期事件 / UID / 折行 / 生成 .ics（纯逻辑）
